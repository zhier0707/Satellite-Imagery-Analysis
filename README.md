...
> 当前迭代：完整 **6-Phase** 全栈（用户系统 / 训练 / 转换 / 前端路由化 / 联调 / 文档）+ **核心体验重塑 6-Phase**（价值主张 / 3D 大屏 / 架构修复 / Bug 群 / UI 细节 / 测试与文档）

## 项目价值主张

> **一句话定位**：让「卫星图像分类 + Grad-CAM 可解释 + 高德可视化 + 时相对比」这一整套能力，从"能跑"走到"能演示、能路演、能拿去做团队周会"。

平台在 EuroSAT 10 类土地利用上提供 EfficientNetV2-M 推理，配套 Grad-CAM 热力图、时相变化检测、高德地图分类点展示与 3D 数据驾驶舱；登录后默认进入 `/app/home` 仪表盘，5 张特性卡片 + 「3D 大屏」快捷入口，5 分钟即可走完"上传 → 分类 → 地图 → 报表"完整链路。

## 快速开始

> 完整启动流程见下方「快速开始」章节；这里给 30 秒走完的 happy path。

1. **注册 / 登录**（首位注册者自动获得 admin）→ 浏览器打开 `http://localhost:5173/login`
2. **首页 `/app/home`**：5 张特性卡片引导，含「3D 数据驾驶舱」推荐入口
3. **图像上传 `/app/upload`**：拖一张 64×64 卫星图 → 看到 Top-5 概率 + 自动跳 `/app/top5`
4. **热力图 `/app/heatmap`**：切换 0-9 任意目标类别，重新生成 Grad-CAM 叠图
5. **时相对比 `/app/change`**：上传「同区域 + 不同时间」两张图，看类别变化与差异列表
6. **地图视图 `/app/map`**：在 AMap 上看到全部历史分类点（蓝色脉冲），可搜周边、地理编码、生成分享卡
7. **我的报表 `/app/reports`**：导出 PDF / Excel / CSV 三种格式
8. **3D 大屏 `/app/screen`**：F 键全屏，演示场景首选

## 3D 数据驾驶舱

> 登录 → 首页 → 第 5 张特性卡「3D 数据驾驶舱」点击进入，或直接访问 `/app/screen`。
> 完整使用指南见 **[`docs/SCREEN_GUIDE.md`](docs/SCREEN_GUIDE.md)**。

### 视觉效果

- **3D 地球**（Three.js）：半径 50，64×64 段，自动绕 Y 轴 0.05 rad/s 旋转
- **大气层**：透明蓝绿渐变（BackSide）
- **3 条卫星轨道环**：不同倾斜角，缓慢旋转
- **1000 颗星空背景**：随机分布的星点
- **分类点云**：Top-50 分类点按 10 类霓虹色散布地球表面，size 用 `sin(time + offset)` 周期呼吸
- **配色**：深空黑底 `#0A0E1A` + 霓虹蓝 `#00E5FF` / 青绿 `#10F2C5` + 数字辉光

### 数据层

- **4 个 KPI 卡片**：总记录数 / 今日新增 / 活跃用户 / 平均准确率（60px 衬线大字 + 数字辉光 + 单位标签 + 1.2s 数字滚动）
- **2 张 ECharts**：分类分布环形图（10 类） + 30 天趋势折线图
- **AMap 缩略图**：右下角嵌入 AMap 2D 全球视野（zoom=3），Top-50 蓝色脉冲点
- **状态栏**：实时时间（HH:MM:SS）+ 快捷键提示

### 快捷键

- `F` — 全屏
- `R` — 手动刷新数据
- `Esc` — 退出大屏（先退出全屏，再返回首页）

### 演示场景

- **学术报告开场**：F 全屏后定格 30s，地球旋转 + KPI 滚动抓住观众
- **路演 5 分钟**：R 键手动刷数据展示活跃度
- **团队周会**：全屏模式作为背景墙，配合讲解

### 数据源

`GET /api/stats/dashboard`（30s 服务端缓存；admin 看全平台，user 仅看自己）。
